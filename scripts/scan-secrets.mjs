...
//
// Usage:
//   node scripts/scan-secrets.mjs [path]   # scan a directory (default: cwd)
//   → exit 0 if clean, exit 1 if findings
//
// This is deliberately conservative — false positives are better than
// committing a real Stripe key. Users can add exceptions in a
// `.bodega-secrets-ignore` file if needed (future).
...
const patterns = [
  ...
  },
  {
    ...
    re: /\bsk_test_[a-zA-Z0-9]{24,}\b/,
    severity: 'high',
  },
  {
    name: 'Stripe restricted key',
    re: /\brk_(live|test)_[a-zA-Z0-9]{24,}\b/,
    severity: 'critical',
  },
  {
  ...
  {
    name: 'Vercel token',
    re: /\bvercel[_-]?token["'\s:=]+[a-zA-Z0-9]{24,}/i,
    severity: 'high',
  },
  {
    name: 'OpenAI API key',
...
/** Allowlisted filenames — these contain example patterns by design. */
...
function walk(dir, results = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      if (textExtensions.has(ext) || entry.name === '.env') {
        results.push(p);
      }
    }
  }
...
function main() {
  ...
  for (const f of findings) {
    console.error(
      `  ${f.severity.toUpperCase().padEnd(8)} ${f.name}`,
    );
    console.error(`           ${f.file}:${f.line} — ${f.preview}\n`);
  }
  console.error(
    `${findings.length} finding(s). Fix before pushing. If any is a false positive,`,
  );
  console.error('add the filename to the allowlist in scripts/scan-secrets.mjs.');
  process.exit(1);
}

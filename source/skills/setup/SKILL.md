...
2. Check for `.bodega.md`. If it exists, this project is already set up.
   ...
   `{{command_prefix}}bodega:status` to see current state, or
   `{{command_prefix}}bodega:reconfigure` to change voice or beneficiary."* Exit.
3. **Run preflight (doctor)**: invoke `{{command_prefix}}bodega:doctor`.
   - If doctor exits non-zero (critical failures): halt. Show the doctor
     ...
     re-running `{{command_prefix}}bodega:setup`.
...
`{{scripts_path}}/../reference/bodega-config.example.md`.
...
Invoke `{{command_prefix}}bodega:greenfield-design`. Wait for completion.
...
Invoke `{{command_prefix}}bodega:hosting`. Wait for completion.
`.bodega.md` → `state.hosting: done`.

If this step fails (user declines Vercel login, network issue, etc.), write
state and exit cleanly. User can resume with `{{command_prefix}}bodega:setup`
later.

## Step 6 — Payments

Invoke `{{command_prefix}}bodega:payments`. Stripe KYC can take 10 min to
3 days. Handle both:
...
Invoke `{{command_prefix}}bodega:deploy`. Scaffolds storefront routes,
...
Invoke `{{command_prefix}}bodega:admin`. Provisions `/studio`, wires
...
- Now → invoke `{{command_prefix}}bodega:domain`.
- Later → skip; user runs `{{command_prefix}}bodega:domain` whenever.
...
Yes → invoke `{{command_prefix}}bodega:backup`. No → skip.
...
- **Resume-ability**: if any step fails or the user exits, write state
  to `.bodega.md` so `{{command_prefix}}bodega:setup` resumes where it
  stopped.

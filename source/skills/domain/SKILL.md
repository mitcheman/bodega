...
> `{{command_prefix}}bodega:domain` again when you've added them — I'll
> check if it's ready.
...
>
> Re-run `{{command_prefix}}bodega:domain` to verify after propagation.
...
Since DNS can take up to 24 hours, `{{command_prefix}}bodega:domain` is
run twice:

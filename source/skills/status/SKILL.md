...
> `{{command_prefix}}bodega:setup` to start.
...
- `payments: pending` → "Your partner hasn't finished Stripe. Nudge them
  or run `{{command_prefix}}bodega:payments` when ready."
...
- `admin` last_seen > 30 days → "Your merchant hasn't logged in recently.
  Send them a fresh link with `{{command_prefix}}bodega:invite`."
...
>
> Want me to try deploying again? Run `{{command_prefix}}bodega:deploy`.
